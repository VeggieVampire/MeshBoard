...
def load_config(path=CONFIG_PATH):
    ...
    config = autogenerated_admin_config(os.path.dirname(os.path.abspath(path)), config)
    return config


def autogenerated_admin_config(app_dir=APP_DIR, base=None):
    config = dict(base or {})
    config.setdefault("enabled", False)
    config.setdefault("host", "0.0.0.0")
    config["port"] = int(config.get("port") or 8080)
    config.setdefault("username", "sysop")
    config.setdefault("password_hash", "")
    config.setdefault("session_secret", "change-this-random-secret")
    config["database"] = {"path": os.path.join(app_dir, "meshboard.db")}
    config["mesh_config_path"] = os.path.join(app_dir, mesh_config.CONFIG_FILE)
    config["wifi_remote_config_path"] = os.path.join(app_dir, "wifi_remote.conf")
    config["app_dir"] = app_dir
    config["games_dir"] = os.path.join(app_dir, "modules", "Games")
    return config
...
<button>Login</button>
    ...
    def show_config(self, error="", values=None):
        ...
        admin_urls_text = ", ".join(admin_urls) if admin_urls else f"http://<pi-ip>:{admin_port}"
        generated_rows = [
            ("Admin URLs", admin_urls_text),
            ("Admin Bind", f"{self.config.get('host', '0.0.0.0')}:{admin_port}"),
            ("App Directory", self.config.get("app_dir", APP_DIR)),
            ("Database", resolve_db_path(self.config)),
            ("Mesh Config", config_path),
            ("Remote WiFi Config", wifi_config_path),
            ("Games Directory", resolve_games_dir(self.config)),
        ]
        generated_settings = "<table><tr><th>Setting</th><th>Generated Value</th></tr>"
        for label, generated_value in generated_rows:
            generated_settings += f"<tr><td>{esc(label)}</td><td>{esc(generated_value)}</td></tr>"
        generated_settings += "</table>"
        hotspot_rows = ""
        ...
        body += (
            "<h2>Generated System Settings</h2>"
            "<p class='muted'>These are generated from the install directory, network, and admin defaults. They are shown here so you do not have to guess paths, IPs, or ports.</p>"
